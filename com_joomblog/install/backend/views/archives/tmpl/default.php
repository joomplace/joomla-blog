	...
	<tbody>
		<tr>
			...
			<td valign="top" width="100%" >
				<form action="<?php echo JRoute::_('index.php?option=com_joomblog&view=archives'); ?>" method="post" name="adminForm" >
					<?php if ($this->archives or $this->categories) { ?>
					<fieldset id="filter-bar">
						<?php if ($this->archives or $search) { ?>
					...
					<table class="adminlist">
						...
						<tbody>
						<?php if ($this->archives) {foreach($this->archives as $i => $item) {
							?>
						...
						<?php }} else { ?>
							<tr>
								<td colspan="11" align="center" >
									<?php if ($this->categories) { ?>
									<?php echo JText::sprintf('COM_JOOMBLOG_FIELD_NONE', 'archives'); ?>
