...
class JbblogShowTask extends JbblogShowBase
{
	...
	function display($styleid = '', $wrapTag = 'div')
	{
		...
		$add_reading_blog = JFactory::getApplication()->input->get('reading_blog', '', 'POST');

		if ( !empty($add_reading_blog) ) {
			$query = "SELECT `reading_list` FROM #__joomblog_user WHERE user_id = '" . (int)$my->id . "' LIMIT 1";
			$db->setQuery($query);
			$old_reading_list = $db->loadResult();
			if ( !empty($old_reading_list) ) {
				$new_reading_list = $old_reading_list . ',' . $add_reading_blog;
			} else {
				$new_reading_list = $add_reading_blog;
			}

			$query = "UPDATE #__joomblog_user SET `reading_list`='" . $new_reading_list . "' WHERE user_id = '" . (int)$my->id . "'";
			$db->setQuery($query);
			$db->execute();
			$post_link = str_replace('amp;', '', $row->permalink);
			$mainframe->redirect(JRoute::_($post_link), JText::_('COM_JOOMBLOG_BLOG_ADDED_TO_READING_LIST') . ' <a href="' . JRoute::_(Juri::root() . 'index.php?option=com_joomblog&task=dashboard&Itemid=' . $Itemid . '#jb-reading-list', false) . '">' . JText::_('COM_JOOMBLOG_USER_DASHBOARD_READING_LIST') . '</a>');
		}
		if ( isset($this->row) ) $row = & $this->row; else $row = null;
		if ( !isset($row->id) ) {
			return JText::_('COM_JOOMBLOG_BLOG_ADMIN_NO_ENTRIES');
		...
		if ( !($row->id) ) $row->id = JFactory::getApplication()->input->get('show');


		$date = JFactory::getDate();

		if ( $row->state != 1 || $row->publish_up > $date->toSql() ) {
		...
		if ( !$tpl->is_cached() ) {
			...
			if ( $matches ) {
				$embed_scripts =
					'function show_social(embed_id,social_id) { if(social_id.style.display=="none") {  social_id.style.display="block"; embed_id.style.display="none";  FB.XFBML.parse(); }  else {  social_id.style.display="none"; }  }'."\n"
					.'function show_embed(embed_id,input_id,social_id) { if(embed_id.style.display=="none") {  embed_id.style.display="block"; social_id.style.display="none"; $(input_id).select(); }  else {  embed_id.style.display="none";  }  }'."\n"
					.'function show_only_embed(embed_id,input_id) { if(embed_id.style.display=="none") {  embed_id.style.display="block"; $(input_id).select(); }  else {  embed_id.style.display="none";  }  }'."\n"
					.'function show_only_social(social_id) { if(social_id.style.display=="none") {  social_id.style.display="block"; FB.XFBML.parse();  }  else {  social_id.style.display="none"; }  }';
				$doc->addScriptDeclaration($embed_scripts);

				$j = 1;
				foreach ($matches[0] as $match) {
					(strpos($match, 'data-embed="1"')) ? $img_embed = true : $img_embed = false;
					(strpos($match, 'data-facebook="1"')) ? $img_facebook = true : $img_facebook = false;
					(strpos($match, 'data-twitter="1"')) ? $img_twitter = true : $img_twitter = false;
					(strpos($match, 'data-pinterest="1"')) ? $img_pinterest = true : $img_pinterest = false;
					(strpos($match, 'data-linkedin="1"')) ? $img_linkedin = true : $img_linkedin = false;
					(strpos($match, 'data-googleplus="1"')) ? $img_googleplus = true : $img_googleplus = false;
					if ( $img_embed OR $img_facebook OR $img_twitter OR $img_pinterest OR $img_linkedin OR $img_googleplus ) {
						...
						$embed_content = str_replace('>', '&gt;', $embed_content);
						$embed_content = preg_replace('/ {2,}/', ' ', $embed_content);
						...
						<a href="//pinterest.com/pin/create/button/?url=' . $post_link . '&media=' . $image_link . '&description=' . $title . '" data-pin-do="buttonPin" data-pin-config="beside"><img src="//assets.pinterest.com/images/pidgets/pin_it_button.png" /></a>
						</div>';
						}
						//add twitter button
			...
			$query = "SELECT `defaultimage` FROM #__joomblog_posts WHERE `id` = '" . (int)$row->id . "' ";
			$db->setQuery($query);
			$row->defaultimage = $db->loadResult();
			...
			if ( $usefacebook ) {
				...
				if ( $showFbInPost ) {
					...
					$row->fb_button = '<div id="fb-root"></div><!--<div class="fb-recommendations-bar" data-href="' . $fbUrl . '" data-trigger="50%" data-read-time="20" num_recommendations="5"></div>-->';

					switch ($fbStyle) {
						case 'none':
							$row->fb_button .= '<fb:like href="' . $fbUrl . '" send="' . ($fbSendButton ? true : false) . '" width="' . $fbwidth . '" show_faces="false" font=""></fb:like>';
							break;
						case 'horizontal':
							$row->fb_button .= '<fb:like layout="button_count" href="' . $fbUrl . '" send="' . ($fbSendButton ? true : false) . '" width="' . $fbwidth . '" show_faces="false" font=""></fb:like>';
							break;
						case 'vertical':
							$row->fb_button .= '<fb:like layout="box_count" href="' . $fbUrl . '" send="' . ($fbSendButton ? true : false) . '" width="' . $fbwidth . '" show_faces="false" font=""></fb:like>';
							break;
					}
	...
	private function fbOpengraphMeta($e)
	{
		...
		$doc = JFactory::getDocument();
		$doc->addCustomTag($this->getMetatags($permalink, $meta_title, $meta_image, $meta_description, $meta_updated_time, $fbadmin, $fbappid, $meta_author, $meta_articletags, $meta_articlepublish, $e));
	}

	private function getMetatags($permalink, $meta_title, $meta_image, $meta_description, $meta_updated_time, $fbadmin, $fbappid, $meta_author, $meta_articletags, $meta_articlepublish, $e = array())
	{
		$metatags = '';
		...
		{
			if (!is_array($e['custom_metatags']) ) @unserialize($e['custom_metatags']);
		...
		$introtextStripped = strip_tags($e['text'], '<br><p>');
		$introtextStripped = preg_replace('/<br[^>]*>/isu', "\n", $introtextStripped);
		$introtextStripped = str_replace('</p>', "\n", $introtextStripped);
		$introtextStripped = preg_replace('/<[^>]+>/isu', '', $introtextStripped);
		$introtextStripped = trim( str_replace('&nbsp;', " ", $introtextStripped) );
		$twitterIntro = substr($introtextStripped, 0, strpos($introtextStripped, "\n"));

		// twitter tags
		if ( empty($e['custom_metatags']['twitter:description']) )
			$metatags.='<meta name="twitter:description" content="'.( !empty($meta_description) ? $meta_description : substr($twitterIntro, 0, 200) ).'" />'."\n";

		if ( empty($e['custom_metatags']['twitter:title']) )
			$metatags.='<meta name="twitter:title" content="'.substr($meta_title, 0, 70).'" />'."\n";
